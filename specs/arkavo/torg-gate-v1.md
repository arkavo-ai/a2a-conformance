...
| `data` | §5.4-shaped array of `@type` objects, see below. The JSON-RPC `message` field is the literal string `Refused by policy`; *TorgPolicyRefusal* is the error's registry name only and never appears in the `message` field. Verifiers MUST tolerate additional `data` members beyond the required Rejection object (SDK envelopes may append e.g. `google.rpc.ErrorInfo`). The array MUST contain exactly one `arkavo.torg.v1.Rejection` object, and its `policyId` and `ruleId` MUST be populated — refusals are required to be debuggable |
